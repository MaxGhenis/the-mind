...
def _times_schema() -> dict:
    # Use the common supported subset. Hand length and time bounds are validated
    # locally; Anthropic does not support arbitrary array lengths/numeric bounds.
    return {
        "type": "object",
...
class JSONPolicy:
    ...
    ``structured_output=True`` requests the same numerical-array JSON schema
    through each provider's native structured-output parameter. It requires a
    supporting model and endpoint; failures never fall back to unconstrained text.
    """
    ...
    def descriptor(self) -> dict:
        ...
        if self.structured_output:
            descriptor["response_schema"] = _times_schema()
        return descriptor
        ...
        if self.structured_output:
            if self.provider == "anthropic":
                payload["output_config"] = {
                    "format": {"type": "json_schema", "schema": _times_schema()}
                }
            else:
